...
const sampleData = {
  ...
  profile: {
    ...
    name: 'Dongyan',
    title: '独立开发者',
    email: '2303201737@qq.com',
    github: 'Dongyan0326',
    phone: '+86 180 1373 3907',
    about: '作为一名怀揣物理思维与AI热情的学习者，我始终相信科学的交叉融合将重塑未来。本科阶段的物理学训练赋予我严谨的逻辑推导能力与对自然规律的敏锐洞察力，而当下对Web开发、程序设计的探索，则让我掌握了将抽象概念转化为实际应用的桥梁。\n\n在学习HTML、CSS和JavaScript构建交互界面的过程中，我逐渐理解用户需求与技术实现间的平衡艺术；通过Python程序设计，我培养出结构化解决问题的思维方式，这些都为我的AI探索之路奠定了坚实基础。',
    photo_url: '/assets/images/portrait.jpg',
    skills: ['JavaScript', 'Python', 'HTML/CSS', 'React', 'Node.js', '数据库设计', '机器学习', 'Web开发']
  },
  ...
  comments: [
    {
      ...
      post_id: 1,
      name: '张明',
      content: 'React入门介绍得很棒！期待更多这样的内容。',
      date: '2023-06-16T08:22:00Z'
    },
    {
      id: 2,
      post_id: 1,
      name: '李华',
      content: '这篇文章帮我更好地理解了React钩子。谢谢分享！',
    ...
    },
    {
      ...
      post_id: 3,
      name: 'Alex Johnson',
      content: 'CSS Grid changed my life! Thanks for explaining it so clearly.',
  ...
  // 访客统计数据
  visitors: [
    { date: '2023-06-15', count: 187 },
    { date: '2023-06-16', count: 256 },
    { date: '2023-06-17', count: 198 },
    { date: '2023-06-18', count: 278 },
    { date: '2023-06-19', count: 235 },
    { date: '2023-06-20', count: 302 },
    { date: '2023-06-21', count: 328 }
  ]
};
...
function exportPosts() {
  fs.writeFileSync(
    ...
    JSON.stringify(sampleData.posts, null, 2)
  );
  console.log(`✅ 已导出 ${sampleData.posts.length} 篇文章`);
}
...
function exportComments() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify(sampleData.comments, null, 2)
  );
  ...
  for (const postId in commentsByPost) {
    fs.writeFileSync(
      path.join(dataDir, `comments_${postId}.json`),
      JSON.stringify(commentsByPost[postId], null, 2)
    );
  }
  
  console.log(`✅ 已导出 ${sampleData.comments.length} 条评论`);
}
...
function exportVisitors() {
  fs.writeFileSync(
    path.join(dataDir, 'visitors.json'),
    JSON.stringify(sampleData.visitors, null, 2)
  );
  console.log(`✅ 已导出访客统计数据（${sampleData.visitors.length}天）`);
}
...
async function exportAll() {
  ...
  try {
    ...
    console.log('✅ 所有示例数据导出完成');
    console.log(`数据文件保存在: ${dataDir}`);
  } catch (err) {
